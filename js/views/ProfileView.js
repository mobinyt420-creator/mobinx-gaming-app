  ...
  return `
    <div class="view-container profile-view">
      ...
      <div class="profile-hero" style="${isAdmin ? 'background: linear-gradient(135deg, #1e1b4b 0%, #1e3a8a 50%, #7c3aed 100%);' : ''}">
        ...
        <div style="font-size: 11.5px; opacity: 0.9;">${user.email}</div>
        <div class="profile-uid-pill">ID: ${user.id} • ${user.role}</div>

        <!-- Google Login / Switch Account Action -->
        <button class="btn-secondary" id="btn-profile-google-login" style="padding: 6px 14px; font-size: 11.5px; margin-top: 6px; border-radius: var(--radius-full); background: rgba(255,255,255,0.95);">
      ...
      <div class="profile-stats-bar">
        <div class="p-stat-item">
          <span class="p-stat-value">${user.stats.tournamentsJoined}</span>
          <span class="p-stat-label">Tournaments</span>
        </div>
        <div class="p-stat-item">
          <span class="p-stat-value">${user.stats.totalDownloads}</span>
        ...
        <div class="p-stat-item">
          <span class="p-stat-value" style="color: var(--success);">$${user.referralEarnings}</span>
