...
function renderGoogleProfileSetupView() {
  return `
    <div class="view-container onboarding-view onboarding-step-2" style="min-height: 100%; height: 100%; box-sizing: border-box; background: #f8fafc; color: #0f172a; padding: 22px 20px calc(24px + var(--safe-bottom)) 20px; display: flex; flex-direction: column; justify-content: space-between; overflow-y: auto; position: relative;">
        ...
        <div style="background: #ffffff; border: 1.5px solid #eef2f6; border-radius: 24px; padding: 22px 18px; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.04); display: flex; flex-direction: column; gap: 16px;">
          ...
          <div>
            ...
            <div id="wrap-input-phone" style="display: flex; align-items: center; border: 1.5px solid #e2e8f0; border-radius: 14px; background: #ffffff; padding: 0 14px; height: 50px; transition: all 0.2s ease;">
              ...
              <input type="tel" id="input-onboard-phone-num" placeholder="01XXXXXXXXX (11 digits)" maxlength="14" style="border: none; outline: none; background: transparent; width: 100%; height: 100%; font-size: 14px; color: #0f172a; font-weight: 600; padding-left: 10px;" />
            </div>
          </div>

          <div style="margin-top: 4px;">
            <button id="btn-continue-google" style="width: 100%; height: 52px; background: #ffffff; border: 2px solid #e2e8f0; border-radius: 16px; display: flex; align-items: center; justify-content: center; gap: 10px; font-size: 15px; font-weight: 800; color: #0f172a; cursor: pointer; transition: all 0.2s ease; box-shadow: 0 4px 14px rgba(0, 0, 0, 0.05);">
      ...
      <!-- Bottom Privacy Policy & Data Safety -->
      <div style="position: relative; z-index: 2; margin-top: 20px; text-align: center;">
        <span style="font-size: 11.5px; color: #64748b;">By continuing, you agree to our </span>
        <a href="javascript:void(0)" id="link-onboard-privacy" style="font-size: 11.5px; font-weight: 700; color: #0066ff; text-decoration: underline;">Privacy Policy & Data Safety</a>
      </div>
...
export function bindOnboardingEvents() {
  ...
  const handleGoogleAuth = async () => {
    ...
    if (fullName.length < 2) {
      Toast.show('Please enter your full name first', 'warning');
      nameInput?.focus();
      return;
    }

    if (phone.length < 10) {
      Toast.show('Please enter a valid 11-digit mobile number', 'warning');
      phoneInput?.focus();
      return;
    }
    ...
    if (btnGoogle) {
      ...
      btnGoogle.innerHTML = `<span style="font-size:14px; font-weight:700;">Connecting with Google...</span>`;
    }
    ...
    try {
      ...
      const user = await authService.loginWithGoogle(email, fullName, phone, '', avatar, uid);
